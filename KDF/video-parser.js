...
// 处理视频数据拦截
if (url.includes('/api/v1/nba/game/') || url.includes('/cms/api/live/studio/id/v4') || url.includes('/cms/api/c/player/common')) {
    handleVideoData();
} 
// 处理自定义HTML页面请求
...
function handleNBAContent(gameData) {
    ...
    storage.set('nba_game_backgroundCover', gameData.backgroundCover || gameData.sourceCover);
    ...
    streams.forEach((stream, index) => {
        ...
        storage.set(streamKey, JSON.stringify({
            title: stream.title,
            cover: stream.cover,
...
function handleTVContent(tvData) {
    ...
    if (tvData.medias && tvData.medias.data) {
        ...
        tvData.medias.data.forEach((episode, index) => {
            ...
            storage.set(episodeKey, JSON.stringify({
...
function sendNotification(title, type) {
    const message = type === "NBA比赛" ? "点击查看比赛直播和集锦" : "点击查看剧集";
    const url = "https://360.com/video"; // 使用360.com域名
    
    // 根据不同代理工具发送通知
    if (isLoon) {
        $notification.post(title, "", message, { "openUrl": url });
    } else if (isQuanX) {
...
function renderTVPage() {
        ...
        if (episodeData) {
            ...
            <div class="episode-item ${isActive ? 'active' : ''}">
                <div class="episode-thumbnail">
                    <img src="${episode.cover}" alt="${episode.title}" onerror="this.src='data:image/svg+xml;base64,无封面</dGV4dD48L3N2Zz4=';">
                </div>
                <h3>${episode.title}</h3>
    ...
    if (currentMedia.qualitys && currentMedia.qualitys.length > 0) {
        qualityBtns = currentMedia.qualitys.map(q => 
            `<a href="${q.qualityUrl}" class="quality-btn" target="_blank">${q.qualityShortName}</a>`
        ).join('');
    }
    ...
    <body>
        ...
        <div class="container">
            <div class="current-episode">
                <h2>${currentMedia.mediaName || ""} - ${currentMedia.mediaSubTitle || ""}</h2>
                <p>选择清晰度:</p>
                <div class="quality-options">
                    ${qualityBtns}
                </div>
