...
export function resolvePlayer(settings) {
	if (isValidCustomScheme(settings.scheme)) return { scheme: settings.scheme, needEncode: true, supportName: false };
...
export function buildLaunchUrl(videoUrl, videoName, settings) {
	...
	if (player.supportName && videoName) {
		...
		if (!cleanName.toLowerCase().endsWith(".mp4")) cleanName = `${cleanName.replace(/\.[^.]+$/, "")}.mp4`;
		finalUrl += `${finalUrl.includes("?") ? "&" : "?"}name=${encodeURIComponent(cleanName)}`;
	}
	return finalUrl;
}
